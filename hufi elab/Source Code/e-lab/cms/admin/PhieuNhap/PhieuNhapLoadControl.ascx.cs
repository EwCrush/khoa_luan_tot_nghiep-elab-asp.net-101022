...
namespace e_lab.cms.admin.PhieuNhap
{
    public partial class PhieuNhapLoadControl : System.Web.UI.UserControl
    {
        ...
        protected int getlast()
        {
            int tonghang = 0;
            DataTable dt = new DataTable();
            dt = e_lab.PhieuNhap.ds_PhieuNhap();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                tonghang++;
            }
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuNhap.ds_PhieuNhap_pagination(getoffset());
            ...
            <td class='tb_mi20 table_info'>" + dt.Rows[i]["NgayLap"] + @"</td>
            ...
            <td class='tb_mi20 table_info'>
                <a href='/Admin.aspx?modul=CTPNHoaChat&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-flask' title='Chi tiết phiếu nhập hóa chất'></i>
                </a>
                <a href='/Admin.aspx?modul=CTPNThietBi&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-microscope' title='Chi tiết phiếu nhập thiết bị'></i>
                </a>    
                <a href='/Admin.aspx?modul=CTPNDungCu&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
