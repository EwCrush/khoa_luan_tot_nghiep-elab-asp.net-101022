<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ChiTietPhieuNhapThietBiLoadControl.ascx.cs" Inherits="e_lab.cms.admin.PhieuNhap.ChiTietPhieuNhapThietBi.ChiTietPhieuNhapThietBiLoadControl" %>
...
<div class="pagination-wrapper">
    <div class="pagination floatleft-item">
        <a href="/Admin.aspx?modul=ThemChiTietPhieuNhapThietBi&mapn=<%=getmapn() %>" class="ThemMoi">
            <i class="to-do-heading-icon fa-solid fa-circle-plus" title="Thêm"></i>
        </a>
    </div>
    <div class="pagination">
        <a href="/Admin.aspx?modul=ChiTietPhieuNhapThietBi&mapn=<%=getmapn() %>&trang=<%=getpre() %>" class="ThemMoi">
        ...
        <a href="/Admin.aspx?modul=ChiTietPhieuNhapThietBi&mapn=<%=getmapn() %>&trang=<%=getnext() %>" class="ThemMoi">
...
<script type="text/javascript">
    function XoaCTPNTB(MaPN, MaTB) {
        if (confirm("Bạn có muốn xóa thiết bị này ra khỏi phiếu nhập chứ?")) {
            $.post("cms/admin/PhieuNhap/ChiTietPhieuNhapThietBi/Ajax/ChiTietPhieuNhapThietBi.aspx",
                {
                    "MaPN": MaPN,
                    "MaTB": MaTB
                },
