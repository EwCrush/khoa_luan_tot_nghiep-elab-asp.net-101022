...
namespace e_lab.cms.admin.PhieuNhap.PhieuNhapDangChoXacNhan
{
    public partial class PhieuNhapDangChoXacNhanLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        protected int getlast()
        {
            ...
            dt = e_lab.PhieuNhap.ds_PhieuNhap_dangxacnhan();
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuNhap.ds_PhieuNhap_dcxn_pagination(getoffset());
            //grid_phonglab.DataSource = dt;
            //grid_phonglab.DataBind();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ltr_ds_tb.Text += @"
                <tr id='Line" + dt.Rows[i]["MaPN"] + @"' class='table_row'>
            <td class='tb_mi15 table_info'>" + dt.Rows[i]["MaPN"] + @"</td>
            ...
            <td class='tb_mi20 table_info'>
                <a href='/Admin.aspx?modul=ChiTietPhieuNhapHoaChat&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-flask' title='Chi tiết phiếu nhập hóa chất'></i>
                </a>
                <a href='/Admin.aspx?modul=ChiTietPhieuNhapThietBi&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-microscope' title='Chi tiết phiếu nhập thiết bị'></i>
                </a>    
                <a href='/Admin.aspx?modul=ChiTietPhieuNhapDungCu&mapn=" + dt.Rows[i]["MaPN"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-vial' title='Chi tiết phiếu nhập dụng cụ'></i>
                </a>
            </td>
            <td class='tb_mi20 table_info'>
                <a href='javascript:XacNhanPN(" + dt.Rows[i]["MaPN"] + @")' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-check' title='Xác nhận'></i>
                </a>
                <a href='/Admin.aspx?modul=SuaPhieuNhap&mapn=" + dt.Rows[i]["MaPN"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaPN(" + dt.Rows[i]["MaPN"] + @")' class='tb_thaotac_link'>
