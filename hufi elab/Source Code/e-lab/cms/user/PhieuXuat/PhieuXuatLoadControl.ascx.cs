...
namespace e_lab.cms.user.PhieuXuat
{
    public partial class PhieuXuatLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        protected int getlast()
        {
            int tonghang = 0;
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuatKhoa.ds_PhieuXuat(Session["MaDonVi"].ToString());
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                tonghang++;
            }
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuatKhoa.ds_PhieuXuat_pagination(Session["MaDonVi"].ToString(), getoffset());
            ...
            <td class='tb_mi20 table_info'>
                <a href='/User.aspx?modul=ChiTietPhieuXuatHoaChat&mapxk=" + dt.Rows[i]["MaPXK"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-flask' title='Chi tiết phiếu nhập hóa chất'></i>
                </a>
                <a href='/User.aspx?modul=ChiTietPhieuXuatThietBi&mapxk=" + dt.Rows[i]["MaPXK"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-microscope' title='Chi tiết phiếu nhập thiết bị'></i>
                </a>    
                <a href='/User.aspx?modul=ChiTietPhieuXuatDungCu&mapxk=" + dt.Rows[i]["MaPXK"] + @"&trang=1' class='tb_thaotac_link'>
