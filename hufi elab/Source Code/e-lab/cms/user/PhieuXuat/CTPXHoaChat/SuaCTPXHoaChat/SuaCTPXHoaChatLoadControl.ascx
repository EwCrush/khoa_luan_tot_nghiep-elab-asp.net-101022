<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SuaCTPXHoaChatLoadControl.ascx.cs" Inherits="e_lab.cms.user.PhieuXuat.CTPXHoaChat.SuaCTPXHoaChat.SuaCTPXHoaChatLoadControl" %>

<div class="to-do-heading">
    <div class="to-do-heading-icon-list">
        <a href="/User.aspx?modul=CTPXHoaChat&mapxk=<%=getmapxk() %>&trang=1" class="Back" <%--style="color:#fff; text-decoration: none"--%>>
            <i class="to-do-heading-icon fa-solid fa-circle-chevron-left" title=" Quay lại"></i>
        </a>
    </div>
    <div class="heading-text">Cập nhật số lượng cho hóa chất <%=getmahc() %> trong phiếu xuất <%=getmapxk() %></div>
</div>
