...
namespace e_lab.cms.user.PhieuXuat.CTPXThietBi
{
    public partial class CTPXThietBiLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuatKhoa.CTPX_tb_pagination(mapxk, getoffset());
            ...
            <td class='tb_mi10 table_info'>
                <a href='/User.aspx?modul=SuaCTPXThietBi&mapxk=" + dt.Rows[i]["MaPXK"] + @"&matb=" + dt.Rows[i]["MaTB"] + @"&mapl=" + dt.Rows[i]["MaPL"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPXTB(" + dt.Rows[i]["MaPXK"] + @", " + dt.Rows[i]["MaTB"] + @", " + dt.Rows[i]["MaPL"] + @")' class='tb_thaotac_link'>
