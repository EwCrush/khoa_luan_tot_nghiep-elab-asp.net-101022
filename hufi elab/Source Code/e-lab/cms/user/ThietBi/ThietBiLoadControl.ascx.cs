...
namespace e_lab.cms.user.ThietBi
{
    public partial class ThietBiLoadControl : System.Web.UI.UserControl
    {
        ...
        protected int getlast()
        {
            int tonghang = 0;
            DataTable dt = new DataTable();
            dt = e_lab.VatTuKhoa.ds_thietbikhoa(Session["MaDonVi"].ToString());
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.VatTuKhoa.ds_thietbikhoa_pagination(Session["MaDonVi"].ToString(), getoffset());
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ltr_ds_dc.Text += @"
