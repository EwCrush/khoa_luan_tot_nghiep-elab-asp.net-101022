...
namespace e_lab.cms.user.NhanVien.TimKiemNhanVien
{
    public partial class TimKiemNhanVienLoadControl : System.Web.UI.UserControl
    {
        ...
        protected int getlast()
        {
            int tonghang = 0;
            DataTable dt = new DataTable();
            dt = e_lab.NhanVien.ds_nv_search_keyword_khoa(keyword, Session["MaDonVi"].ToString());
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.NhanVien.ds_nv_search_keyword_pagination_khoa(keyword, Session["MaDonVi"].ToString(), getoffset());
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ltr_ds_nv.Text += @"
