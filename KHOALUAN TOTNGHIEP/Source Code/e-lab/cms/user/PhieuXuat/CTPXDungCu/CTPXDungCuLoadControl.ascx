<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CTPXDungCuLoadControl.ascx.cs" Inherits="e_lab.cms.user.PhieuXuat.CTPXDungCu.CTPXDungCuLoadControl" %>
...
<div class="to-do-heading">
    ...
    <div class="heading-text">Danh sách dụng cụ trong phiếu xuất <%=getmapxk() %></div>
</div>
...
<div class="pagination-wrapper">
    <div class="pagination floatleft-item">
        <a href="/User.aspx?modul=ThemCTPXDungCu&mapxk=<%=getmapxk() %>" class="ThemMoi">
            <i class="to-do-heading-icon fa-solid fa-circle-plus" title="Thêm"></i>
        </a>
    </div>
    <div class="pagination">
        <a href="/User.aspx?modul=CTPXDungCu&mapxk=<%=getmapxk() %>&trang=<%=getpre() %>" class="ThemMoi">
        ...
        <a href="/User.aspx?modul=CTPXDungCu&mapxk=<%=getmapxk() %>&trang=<%=getnext() %>" class="ThemMoi">
...
<script type="text/javascript">
    function XoaCTPXDC(MaPXK, MaDC) {
        if (confirm("Bạn có muốn xóa dụng cụ này ra khỏi phiếu xuất chứ?")) {
            $.post("cms/user/PhieuXuat/Ajax/XoaCTPXDungCu.aspx",
                {
                    "MaPXK": MaPXK,
                    "MaDC": MaDC
                },
