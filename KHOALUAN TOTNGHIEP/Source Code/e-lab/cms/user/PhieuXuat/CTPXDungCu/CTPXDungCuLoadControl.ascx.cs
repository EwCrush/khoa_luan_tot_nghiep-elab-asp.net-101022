...
namespace e_lab.cms.user.PhieuXuat.CTPXDungCu
{
    public partial class CTPXDungCuLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuatKhoa.CTPX_dc_pagination(mapxk, getoffset());
            ...
            <td class='tb_mi60 table_info'>" + dt.Rows[i]["TenDC"] + @"</td>
            <td class='tb_mi25 table_info'>" + dt.Rows[i]["SoLuong"] + @"</td>
            <td class='tb_mi15 table_info'>
                <a href='/User.aspx?modul=SuaCTPXDungCu&mapxk=" + dt.Rows[i]["MaPXK"] + @"&madc=" + dt.Rows[i]["MaDC"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPXDC(" + dt.Rows[i]["MaPXK"] + @", " + dt.Rows[i]["MaDC"] + @")' class='tb_thaotac_link'>
