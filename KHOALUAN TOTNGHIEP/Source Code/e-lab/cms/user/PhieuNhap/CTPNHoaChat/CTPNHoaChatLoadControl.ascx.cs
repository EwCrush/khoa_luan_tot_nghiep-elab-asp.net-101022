...
namespace e_lab.cms.user.PhieuNhap.CTPNHoaChat
{
    public partial class CTPNHoaChatLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuat.CTPX_hc_pagination(mapx, getoffset());
            ...
            <td class='tb_mi15 table_info'>
                <a href='/User.aspx?modul=SuaCTPNHoaChat&mapx=" + dt.Rows[i]["MaPX"] + @"&mahc=" + dt.Rows[i]["MaHC"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPNHC(" + dt.Rows[i]["MaPX"] + @", " + dt.Rows[i]["MaHC"] + @")' class='tb_thaotac_link'>
