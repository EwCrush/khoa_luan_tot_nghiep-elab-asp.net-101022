<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SuaCTPNDungCuLoadControl.ascx.cs" Inherits="e_lab.cms.user.PhieuNhap.CTPNDungCu.SuaCTPNDungCu.SuaCTPNDungCuLoadControl" %>

<div class="to-do-heading">
    <div class="to-do-heading-icon-list">
        <a href="/User.aspx?modul=CTPNDungCu&mapx=<%=getmapx() %>&trang=1" class="Back" <%--style="color:#fff; text-decoration: none"--%>>
            <i class="to-do-heading-icon fa-solid fa-circle-chevron-left" title=" Quay lại"></i>
        </a>
    </div>
    <div class="heading-text">Cập nhật số lượng cho dụng cụ <%=getmadc() %> trong phiếu nhập <%=getmapx() %></div>
</div>
