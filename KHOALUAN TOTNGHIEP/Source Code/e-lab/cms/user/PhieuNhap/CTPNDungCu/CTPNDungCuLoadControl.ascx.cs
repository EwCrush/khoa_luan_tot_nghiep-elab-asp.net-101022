...
namespace e_lab.cms.user.PhieuNhap.CTPNDungCu
{
    public partial class CTPNDungCuLoadControl : System.Web.UI.UserControl
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuat.CTPX_dc_pagination(mapx, getoffset());
            ...
            <td class='tb_mi15 table_info'>
                <a href='/User.aspx?modul=SuaCTPNDungCu&mapx=" + dt.Rows[i]["MaPX"] + @"&madc=" + dt.Rows[i]["MaDC"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPNDC(" + dt.Rows[i]["MaPX"] + @", " + dt.Rows[i]["MaDC"] + @")' class='tb_thaotac_link'>
