...
namespace e_lab.cms.admin.ThietBi.ThongSo
{
    public partial class ThongSoLoadControl : System.Web.UI.UserControl
    {
        ...
        private void layDSTB()
        {
            ...
            <td class='tb_mi15 table_info'>" + dt.Rows[i]["MaTS"] + @"</td>
            <td class='tb_mi70 table_info'>" + dt.Rows[i]["ThongSo"] + @"</td>
            <td class='tb_mi15 table_info'>
                <a href='/Admin.aspx?modul=SuaThongSo&matb=" + dt.Rows[i]["MaTB"] + @"&mats=" + dt.Rows[i]["MaTS"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaTS(" + dt.Rows[i]["MaTS"] + @")' class='tb_thaotac_link'>
