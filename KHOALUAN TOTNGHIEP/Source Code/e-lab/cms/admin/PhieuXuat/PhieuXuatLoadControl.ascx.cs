...
namespace e_lab.cms.admin.PhieuXuat
{
    public partial class PhieuXuatLoadControl : System.Web.UI.UserControl
    {
        ...
        protected int getlast()
        {
            int tonghang = 0;
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuat.ds_PhieuXuat();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                tonghang++;
            }
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuXuat.ds_PhieuXuat_pagination(getoffset());
            ...
            <td class='tb_mi15 table_info'>
                <a href='/Admin.aspx?modul=ChiTietPhieuXuatHoaChat&mapx=" + dt.Rows[i]["MaPX"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-flask' title='Chi tiết phiếu xuất hóa chất'></i>
                </a>
                <a href='/Admin.aspx?modul=ChiTietPhieuXuatThietBi&mapx=" + dt.Rows[i]["MaPX"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-microscope' title='Chi tiết phiếu xuất thiết bị'></i>
                </a>    
                <a href='/Admin.aspx?modul=ChiTietPhieuXuatDungCu&mapx=" + dt.Rows[i]["MaPX"] + @"&trang=1' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-solid fa-vial' title='Chi tiết phiếu xuất dụng cụ'></i>
                </a>
            </td>
