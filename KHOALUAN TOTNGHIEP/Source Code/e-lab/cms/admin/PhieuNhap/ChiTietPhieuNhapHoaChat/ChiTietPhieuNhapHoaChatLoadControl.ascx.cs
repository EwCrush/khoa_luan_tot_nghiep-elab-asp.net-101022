...
namespace e_lab.cms.admin.PhieuNhap.ChiTietPhieuNhapHoaChat
{
    public partial class ChiTietPhieuNhapHoaChatLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        {
            DataTable dt = new DataTable();
            dt = e_lab.PhieuNhap.CTPN_hc_pagination(mapn, getoffset());
            ...
            <td class='tb_mi15 table_info'>
                <a href='/Admin.aspx?modul=SuaChiTietPhieuNhapHoaChat&mapn=" + dt.Rows[i]["MaPN"] + @"&mahc=" + dt.Rows[i]["MaHC"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPNHC(" + dt.Rows[i]["MaPN"] + @", " + dt.Rows[i]["MaHC"] + @")' class='tb_thaotac_link'>
