...
namespace e_lab.cms.admin.PhieuNhap.ChiTietPhieuNhapDungCu
{
    public partial class ChiTietPhieuNhapDungCuLoadControl : System.Web.UI.UserControl
    {
        string trang = "";
        int trangcuoi = 0;
        int trangtruoc = 0;
        ...
        private void layDS()
        {
            ...
            <td class='tb_mi15 table_info'>
                <a href='/Admin.aspx?modul=SuaChiTietPhieuNhapDungCu&mapn=" + dt.Rows[i]["MaPN"] + @"&madc=" + dt.Rows[i]["MaDC"] + @"' class='tb_thaotac_link'>
                 <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                </a>
                <a href='javascript:XoaCTPNDC(" + dt.Rows[i]["MaPN"] + @", " + dt.Rows[i]["MaDC"] + @")' class='tb_thaotac_link'>
